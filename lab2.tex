...
\usepackage{amsmath,amssymb,graphicx,fancyhdr}

\pagestyle{fancy} 
\fancyhf{} 
\rhead{\leftmark}    %\leftmark=current section, \rightmark= current subsection
\cfoot{\thepage}   


\title{\textsc{This is My First LaTeX Document\\\small{Lab2 Practice}}}
\author{\Large{He SUN}\\\\20411847}   %Large is bigger than large
\date{\today}
...
\subsection{Figure}                %can use \newpage to make a new page
...
\begin{figure}[h]     %h=here, t= top, picture must be in the same folder
\centering
\includegraphics[scale=0.5]{Euler} %0.5 means become a half
...
\begin{equation*}          %* cancels the equation number
\Delta    
\begin{cases}
>0 & \textrm{two real roots,} \quad x_{1,2} = \frac{-b\pm \sqrt{\Delta}}{2a}\ \\\\     %\pm plus minus, \frac...\ fraction
...
\end{cases}
\end{equation*}           %* cancels the equation number
